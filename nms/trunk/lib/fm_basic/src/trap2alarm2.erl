...
lookup(Node,Enterprise,Trap,VBValues) ->
    ...
    F=fun() ->
	      mnesia:select(trap2alarm,MS)
      end,
    Res=mnesia:transaction(F),
    case Res of
	{atomic,[]} ->
	    not_found;
	{atomic,[T]} when is_tuple(T) ->
	    T;
	{atomic,L} ->
	    Valid=filterVB(VBValues,L),
	    Res=hd(lists:sort(fun (X,Y) -> X>Y end, Valid));
	X ->
	    io:format("lookup failed: ~w~n",[X]),
	    not_found
    end.
...
filterVB(VBValues,L) ->
    lists:filter(fun ({_Key,VB,_,_,_,_,_,_}) ->
			 vbTest(VBValues,VB)
		 end, L).
